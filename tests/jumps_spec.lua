---@diagnostic disable: invisible
local helpers = require("script.test_utils")

local M = { name = "jumps" }

function M.run()
    helpers.run_test_case("jumps_basic", function()
        local dir_path = helpers.create_temp_dir()
        local file_path = vim.fs.joinpath(dir_path, "jumps.txt")
        helpers.write_file(file_path, "one\ntwo\nthree\n")
        local buf = helpers.create_named_buffer(file_path, {
            "one",
            "two",
            "three",
        })
        vim.api.nvim_set_current_buf(buf)
        local jump_list = {
            { bufnr = buf, lnum = 2, col = 1, nr = 1 },
        }
        helpers.with_mock(vim.fn, "getjumplist", function()
            return { jump_list, 1 }
        end, function()
            local jumps_picker = require("fuzzy.pickers.jumps")
            local picker = jumps_picker.open_jumps_picker({
                preview = false,
                icons = false,
                prompt_debounce = 0,
            })
            helpers.wait_for_stream(picker)
            helpers.wait_for_list(picker)
            helpers.wait_for_line_contains(picker, "jumps.txt")
            helpers.wait_for_line_contains(picker, ":")
            helpers.wait_for_list_extmarks(picker)
            local extmarks = helpers.get_list_extmarks(picker)
            helpers.assert_has_hl(extmarks, "Number", "jumps prefix hl")
            helpers.assert_has_hl(extmarks, "Directory", "jumps path hl")
            helpers.assert_has_hl(extmarks, "Function", "jumps name hl")
            helpers.assert_has_hl(extmarks, "Number", "jumps line hl")
            picker:close()
        end)

        vim.api.nvim_buf_delete(buf, { force = true })
    end)
end

return M
